...
import DashboardWrapper from "../../components/managerDashboardlayout";
import {
  Eye,
  EyeOff,
  Lock,
...
const Profile = () => {
  ...
  const fixDatabaseSchema = async () => {
    try {
      setLoading((p) => ({ ...p, fixingSchema: true }));
      const response = await axios.post(`${API_BASE_URL}/users/fix-schema`);
      if (response.data.success) {
        showNotification("Core schema synchronization successful", "success");
        checkDatabaseStatus();
        loadUserData();
      }
    } catch (error) {
      showNotification("Schema synchronization failed", "error");
    } finally {
      setLoading((p) => ({ ...p, fixingSchema: false }));
    }
  };
  ...
  return (
    <DashboardWrapper>
      <div className="min-h-screen bg-slate-50/50 p-6 lg:p-10">
          ...
          <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8">
            <div className="space-y-2">
              ...
              <p className="text-lg text-slate-500 font-medium max-w-2xl">
                Elevated access profile. Manage organizational credentials and
                monitor system-wide database health.
              </p>
            </div>
            ...
            <AnimatePresence>
              {(notifications.success ||
                ...
                <motion.div
                  ...
                  exit={{ opacity: 0, x: 20 }}
                  className={`px-6 py-4 rounded-[1.5rem] shadow-xl flex items-center gap-3 text-white font-black text-xs uppercase tracking-widest ${notifications.success ? "bg-emerald-600" : notifications.error ? "bg-rose-600" : "bg-indigo-600"}`}
                >
                  {notifications.success ? (
                    <CheckCircle size={18} />
                  ) : (
        ...
        {/* Database Health Warning (Manager Only) */}
        <AnimatePresence>
          {showSchemaFix && (
            <motion.div
              initial={{ opacity: 0, scale: 0.95 }}
              animate={{ opacity: 1, scale: 1 }}
              className="mb-8 p-8 bg-amber-50 border border-amber-200 rounded-[2.5rem] flex flex-col md:flex-row items-center justify-between gap-6"
            >
              <div className="flex items-center gap-6 text-center md:text-left flex-col md:flex-row">
                ...
                <div>
                  ...
                  <p className="text-amber-700 font-medium">
                    Database schema misalignment detected. System functionality
                    may be compromised.
                  </p>
                </div>
              </div>
              <button
                ...
                disabled={loading.fixingSchema}
                className="px-8 py-4 bg-amber-600 hover:bg-amber-700 text-white rounded-2xl font-black text-xs uppercase tracking-widest transition-all shadow-xl shadow-amber-200 flex items-center gap-3 whitespace-nowrap"
              >
                {loading.fixingSchema ? (
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            >
              ...
              <h2 className="text-2xl font-black text-slate-900 tracking-tight">
                {user.full_name || "Manager Officer"}
              </h2>
              ...
              <div className="mt-10 pt-8 border-t border-slate-50 space-y-4">
                ...
                {databaseStatus && (
                  <div className="flex items-center gap-4 text-slate-500">
                    ...
                    <span className="text-sm font-bold">
                      Node: {databaseStatus.status || "Operational"}
                    </span>
                  </div>
                )}
              </div>
                  ...
                  <form onSubmit={handleUpdateProfile} className="space-y-8">
                    ...
                    <div className="p-10 bg-slate-50 rounded-[2.5rem] border border-slate-100 space-y-6">
                      ...
                      <button
                        ...
                        className="w-full py-5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-[1.5rem] font-black text-xs uppercase tracking-widest shadow-xl shadow-indigo-100 transition-all hover:scale-[1.02] active:scale-[0.98] flex items-center justify-center gap-3"
                      >
                ...
                >
                  <div className="flex items-center gap-6 mb-12">
                    ...
                    <div>
                      ...
                      <p className="text-slate-500 font-medium">
                        Update high-level system entry codes
                      </p>
                    </div>
                  </div>
                ...
                >
                  <div className="flex items-center gap-6 mb-12">
                    ...
                    <div>
                      ...
                      <p className="text-slate-500 font-medium">
                        Review system-wide administrative logs
                      </p>
                    </div>
                  </div>
